...
system.]
  ...
  Synopsis    [Visualization procedures using DOT software
and GSView.]
  ...
  Revision    [$Id: abcShow.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
void Abc_NodeShowBdd(Abc_Obj_t *pNode) {
  ...
  Cudd_DumpDot((DdManager *)pNode->pNtk->pManFunc, 1,
               (DdNode **)&pNode->pData,
...
/**Function*************************************************************
  ...
  Synopsis    [Visualizes a reconvergence driven cut at the
node.]
...
***********************************************************************/
void Abc_NodeShowCut(Abc_Obj_t *pNode, int nNodeSizeMax,
                     int nConeSizeMax) {
  FILE *pFile;
  char FileNameDot[200];
  Abc_ManCut_t *p;
  ...
  p = Abc_NtkManCutStart(nNodeSizeMax, nConeSizeMax, 2,
                         ABC_INFINITY);
  ...
  vNodesTfo = Abc_NodeCollectTfoCands(p, pNode, vCutSmall,
                                      ABC_INFINITY);
  ...
  // write the DOT file
  Io_WriteDotNtk(pNode->pNtk, vInside, vCutSmall,
                 FileNameDot, 0, 0);
...
***********************************************************************/
void Abc_NtkShow(Abc_Ntk_t *pNtk0, int fGateNames, int fSeq,
                 int fUseReverse) {
  FILE *pFile;
  Abc_Ntk_t *pNtk;
  Abc_Obj_t *pNode;
  ...
  if (Abc_NtkIsStrash(pNtk0) &&
      Abc_NtkGetChoiceNum(pNtk0)) {
    printf(
        "Temporarily visualization of AIGs with choice "
        "nodes is disabled.\n");
    return;
  }
  // create the file name
  ...
  pNtk->nBarBufs = 0;
  if (fSeq)
    Io_WriteDotSeq(pNtk, vNodes, NULL, FileNameDot,
                   fGateNames, fUseReverse);
  else
    Io_WriteDotNtk(pNtk, vNodes, NULL, FileNameDot,
                   fGateNames, fUseReverse);
...
void Abc_ShowFile(char *FileNameDot) {
  ...
  sprintf(CommandDot, "%s -Tps -o %s %s", pDotName,
          FileNamePs, FileNameDot);
...
#ifdef WIN32
  _unlink(FileNameDot);
  if (_spawnl(_P_NOWAIT, pGsNameWin, pGsNameWin, FileNamePs,
              NULL) == -1)
    if (_spawnl(_P_NOWAIT,
                "C:\\Program "
      ...
      if (_spawnl(_P_NOWAIT,
                  "C:\\Program "
                  "Files\\Ghostgum\\gsview\\gsview64.exe",
                  "C:\\Program "
                  "Files\\Ghostgum\\gsview\\gsview64.exe",
                  FileNamePs, NULL) == -1) {
...
void Abc_NtkWriteFlopDependency(Abc_Ntk_t *pNtk,
  ...
  fprintf(pFile,
          "# Flop dependency for \"%s\" generated by ABC "
          "on %s\n",
  ...
  Abc_NtkForEachLatchInput(pNtk, pObj, i) {
    vSupp = Abc_NtkNodeSupport(pNtk, &pObj, 1);
    Count = 0;
    Vec_PtrForEachEntry(Abc_Obj_t *, vSupp, pTemp,
                        k) if (!Abc_ObjIsPi(pTemp))
        fprintf(pFile, "  %4d -> %4d\n", pTemp->iTemp, i);
    Vec_PtrFree(vSupp);
  }
  fprintf(pFile, "}\n");
