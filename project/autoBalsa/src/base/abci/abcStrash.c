...
system.]
  ...
  Revision    [$Id: abcStrash.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
static void Abc_NtkStrashPerform(Abc_Ntk_t *pNtk,
                                 Abc_Ntk_t *pNtkNew,
                                 int fAllNodes,
                                 int fRecord);
...
/**Function*************************************************************
  ...
  Description [Because of the structural hashing, this
procedure should not
  change the number of nodes. It is useful to detect the
bugs in the original AIG.]
...
Abc_Ntk_t *Abc_NtkRestrash(Abc_Ntk_t *pNtk, int fCleanup) {
  ...
  if (Abc_NtkGetChoiceNum(pNtk))
    printf(
        "Warning: The choice nodes in the original AIG are "
        "removed by strashing.\n");
  // start the new network (constants and CIs of the old
  // network will point to the their counterparts in the new
  // network)
  pNtkAig =
      Abc_NtkStartFrom(pNtk, ABC_NTK_STRASH, ABC_FUNC_AIG);
  // restrash the nodes (assuming a topological order of the
  // old network)
  ...
  //        printf( "Warning: The network has %d
  //        self-feeding latches.\n",
  //        Abc_NtkCountSelfFeedLatches(pNtkAig) );
  // perform cleanup if requested
  if (fCleanup && (nNodes = Abc_AigCleanup(
                       (Abc_Aig_t *)pNtkAig->pManFunc))) {
    //        printf( "Abc_NtkRestrash(): AIG cleanup
    //        removed %d nodes (this is a bug).\n", nNodes
    //        );
  }
  // duplicate EXDC
  ...
  //    if ( RetValue =
  //    Abc_NtkRemoveSelfFeedLatches(pNtkAig) )
  //        printf( "Modified %d self-feeding latches. The
  //        result may not verify.\n", RetValue );
  return pNtkAig;
}

/**Function*************************************************************

  Synopsis    [Performs structural hashing by generating
random number.]
...
void Abc_NtkRestrashRandom_rec(Abc_Ntk_t *pNtk,
  ...
  pObj->pCopy = Abc_AigAnd((Abc_Aig_t *)pNtk->pManFunc,
...
Abc_Ntk_t *Abc_NtkRestrashRandom(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachCo(pNtk, pObj, i) Abc_NtkRestrashRandom_rec(
      pNtkAig, Abc_ObjFanin0(pObj));
  // finalize the network
  Abc_NtkFinalize(pNtk, pNtkAig);
  // duplicate EXDC
  if (pNtk->pExdc) pNtkAig->pExdc = Abc_NtkDup(pNtk->pExdc);
...
Abc_Ntk_t *Abc_NtkRestrashZero(Abc_Ntk_t *pNtk,
  ...
  Abc_NtkForEachLatch(pNtk, pObj, i) {
    if (Abc_LatchIsInitDc(pObj))
      ...
      Abc_ObjFanout0(pObj)->pCopy =
          Abc_ObjNot(Abc_ObjFanout0(pObj)->pCopy);
  }
  if (Counter)
    printf(
        "Converting %d flops from don't-care to zero "
        "initial value.\n",
        Counter);
  // restrash the nodes (assuming a topological order of the
  // old network)
  Abc_NtkForEachNode(pNtk, pObj, i) pObj->pCopy =
      Abc_AigAnd((Abc_Aig_t *)pNtkAig->pManFunc,
                 Abc_ObjChild0Copy(pObj),
                 Abc_ObjChild1Copy(pObj));
  // finalize the network
  Abc_NtkFinalize(pNtk, pNtkAig);
  // complement the 1-valued registers
  Abc_NtkForEachLatch(pNtkAig, pObj,
                      i) if (Abc_LatchIsInit1(pObj)) {
    Abc_ObjXorFaninC(Abc_ObjFanin0(pObj), 0);
    // if latch has PO as one of its fanouts change latch
    // name
    if (Abc_NodeFindCoFanout(Abc_ObjFanout0(pObj))) {
      Nm_ManDeleteIdName(pObj->pNtk->pManName,
                         Abc_ObjFanout0(pObj)->Id);
      Abc_ObjAssignName(Abc_ObjFanout0(pObj),
  ...
  if (fCleanup && (nNodes = Abc_AigCleanup(
                       (Abc_Aig_t *)pNtkAig->pManFunc)))
    printf(
        "Abc_NtkRestrash(): AIG cleanup removed %d nodes "
        "(this is a bug).\n",
        nNodes);
  // duplicate EXDC
  if (pNtk->pExdc) pNtkAig->pExdc = Abc_NtkDup(pNtk->pExdc);
  // transfer name IDs
  if (pNtk->vNameIds) Abc_NtkTransferNameIds(pNtk, pNtkAig);
  if (pNtk->vNameIds) Abc_NtkUpdateNameIds(pNtkAig);
  // make sure everything is okay
  if (!Abc_NtkCheck(pNtkAig)) {
    printf(
        "Abc_NtkStrash: The network check has failed.\n");
...
/**Function*************************************************************
  ...
  Synopsis    [Transforms logic network into structurally
hashed AIG.]
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkStrash(Abc_Ntk_t *pNtk, int fAllNodes,
                         int fCleanup, int fRecord) {
  Abc_Ntk_t *pNtkAig;
  int nNodes;
  assert(Abc_NtkIsLogic(pNtk) || Abc_NtkIsStrash(pNtk));
  // consider the special case when the network is already
  // structurally hashed
  if (Abc_NtkIsStrash(pNtk))
    return Abc_NtkRestrash(pNtk, fCleanup);
  // convert the node representation in the logic network to
  // the AIG form
  ...
  // duplicate EXDC
  if (pNtk->pExdc)
    pNtkAig->pExdc = Abc_NtkStrash(pNtk->pExdc, fAllNodes,
                                   fCleanup, fRecord);
  // make sure everything is okay
  if (!Abc_NtkCheck(pNtkAig)) {
    printf(
...
/**Function*************************************************************
  ...
  Description [Modifies the first network by adding the
logic of the second.
  Performs structural hashing while appending the networks.
Does not change
  the second network. Returns 0 if the appending failed, 1
otherise.]
...
int Abc_NtkAppend(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2,
  ...
  if (!Abc_NtkCompareSignals(pNtk1, pNtk2, 1, 1))
    printf(
        "Abc_NtkAppend(): The union of the network PIs is "
        "computed (warning).\n");
  // perform strashing
  nNewCis = 0;
  ...
  if (nNewCis)
    printf(
        "Warning: Procedure Abc_NtkAppend() added %d new "
        "CIs.\n",
        nNewCis);
  ...
  if (Abc_NtkIsLogic(pNtk2))
    ...
    Abc_NtkForEachNode(pNtk2, pObj, i) pObj->pCopy =
        Abc_AigAnd((Abc_Aig_t *)pNtk1->pManFunc,
                   Abc_ObjChild0Copy(pObj),
                   Abc_ObjChild1Copy(pObj));
  ...
  if (fAddPos) {
    Abc_NtkForEachPo(pNtk2, pObj, i) {
      ...
      Abc_ObjAssignName(pObj->pCopy, Abc_ObjName(pObj),
                        NULL);
    }
  } else {
    ...
    Abc_NtkForEachCo(pNtk2, pObj, i) {
      iNodeId = Nm_ManFindIdByNameTwoTypes(
          pNtk1->pManName, Abc_ObjName(pObj), ABC_OBJ_PO,
          ABC_OBJ_BI);
      //            if ( iNodeId < 0 )
      //                continue;
      assert(iNodeId >= 0);
      ...
      pDriverNew = Abc_AigOr((Abc_Aig_t *)pNtk1->pManFunc,
                             pDriverOld, pDriverNew);
      if (Abc_ObjRegular(pDriverOld) ==
          Abc_ObjRegular(pDriverNew))
        continue;
      // replace the old driver by the new driver
      fCompl = Abc_ObjRegular(pDriverOld)->fPhase ^
               Abc_ObjRegular(pDriverNew)->fPhase;
      Abc_ObjPatchFanin(
          pObjOld, Abc_ObjRegular(pDriverOld),
          Abc_ObjNotCond(Abc_ObjRegular(pDriverNew),
                         fCompl));
    }
  }
...
/**Function*************************************************************
  ...
  Synopsis    [Transfers the AIG from one manager into
another.]
...
void Abc_NodeStrash_rec(Abc_Aig_t *pMan, Hop_Obj_t *pObj) {
  ...
  if (!Hop_ObjIsNode(pObj) || Hop_ObjIsMarkA(pObj)) return;
  Abc_NodeStrash_rec(pMan, Hop_ObjFanin0(pObj));
  Abc_NodeStrash_rec(pMan, Hop_ObjFanin1(pObj));
  pObj->pData =
      Abc_AigAnd(pMan, (Abc_Obj_t *)Hop_ObjChild0Copy(pObj),
                 (Abc_Obj_t *)Hop_ObjChild1Copy(pObj));
...
Abc_Obj_t *Abc_NodeStrash(Abc_Ntk_t *pNtkNew,
  ...
  if (Abc_NodeIsConst(pNodeOld) ||
      Hop_Regular(pRoot) == Hop_ManConst1(pMan))
    return Abc_ObjNotCond(Abc_AigConst1(pNtkNew),
                          Hop_IsComplement(pRoot));
  // perform special case-strashing using the record of AIG
  // subgraphs
  /*
      if ( fRecord && Abc_NtkRecIsRunning() &&
     Abc_ObjFaninNum(pNodeOld) > 2 &&
     Abc_ObjFaninNum(pNodeOld) <= Abc_NtkRecVarNum() )
      {
          extern Vec_Int_t * Abc_NtkRecMemory();
          extern int Abc_NtkRecStrashNode( Abc_Ntk_t *
     pNtkNew, Abc_Obj_t * pObj, unsigned * pTruth, int nVars
     );
          int nVars = Abc_NtkRecVarNum();
          Vec_Int_t * vMemory = Abc_NtkRecMemory();
          unsigned * pTruth = Hop_ManConvertAigToTruth(
     pMan, Hop_Regular(pRoot), nVars, vMemory, 0 );
          assert( Extra_TruthSupportSize(pTruth, nVars) ==
     Abc_ObjFaninNum(pNodeOld) ); // should be swept
          if ( Hop_IsComplement(pRoot) )
              Extra_TruthNot( pTruth, pTruth, nVars );
          if ( Abc_NtkRecStrashNode( pNtkNew, pNodeOld,
     pTruth, nVars ) )
  ...
  */
  ...
  Abc_NodeStrash_rec((Abc_Aig_t *)pNtkNew->pManFunc,
                     Hop_Regular(pRoot));
  Hop_ConeUnmark_rec(Hop_Regular(pRoot));
  // return the final node
  return Abc_ObjNotCond(
      (Abc_Obj_t *)Hop_Regular(pRoot)->pData,
      Hop_IsComplement(pRoot));
}
...
Abc_Obj_t *Abc_NtkTopmost_rec(Abc_Ntk_t *pNtkNew,
  ...
  if (pNode->pCopy) return pNode->pCopy;
  if (pNode->Level <= (unsigned)LevelCut)
    return pNode->pCopy = Abc_NtkCreatePi(pNtkNew);
  Abc_NtkTopmost_rec(pNtkNew, Abc_ObjFanin0(pNode),
                     LevelCut);
  Abc_NtkTopmost_rec(pNtkNew, Abc_ObjFanin1(pNode),
                     LevelCut);
  return pNode->pCopy =
             Abc_AigAnd((Abc_Aig_t *)pNtkNew->pManFunc,
...
Abc_Ntk_t *Abc_NtkTopmost(Abc_Ntk_t *pNtk, int nLevels) {
  ...
  Abc_NtkCleanCopy(pNtk);
  pObjNew = Abc_NtkTopmost_rec(
      pNtkNew, Abc_ObjFanin0(Abc_NtkPo(pNtk, 0)), LevelCut);
  pObjNew = Abc_ObjNotCond(
      pObjNew, Abc_ObjFaninC0(Abc_NtkPo(pNtk, 0)));
  // add the PO node and name
  pObjPo = Abc_NtkCreatePo(pNtkNew);
  Abc_ObjAddFanin(pObjPo, pObjNew);
  Abc_NtkAddDummyPiNames(pNtkNew);
  Abc_ObjAssignName(pObjPo, Abc_ObjName(Abc_NtkPo(pNtk, 0)),
                    NULL);
  // make sure everything is okay
...
static int Vec_CompareNodeIds(Abc_Obj_t **pp1,
  ...
  if (Abc_ObjRegular(*pp1)->Id < Abc_ObjRegular(*pp2)->Id)
    return -1;
  if (Abc_ObjRegular(*pp1)->Id >
      Abc_ObjRegular(*pp2)->Id)  //
    return 1;
  return 0;
}
...
Vec_Ptr_t *Abc_NodeGetSuper(Abc_Obj_t *pNode) {
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vFront, pAnd, i) {
    ...
    if (Abc_ObjIsNode(pFanin) &&
        !Abc_ObjIsComplement(pFanin) &&
        Abc_ObjFanoutNum(pFanin) == 1)
...
Abc_Ntk_t *Abc_NtkTopAnd(Abc_Ntk_t *pNtk) {
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pObj, i) {
    ...
    pDriver = Abc_ObjNotCond(Abc_ObjRegular(pObj)->pCopy,
  ...
  if ((nNodes =
           Abc_AigCleanup((Abc_Aig_t *)pNtkAig->pManFunc)))
    printf(
        "Abc_NtkTopAnd(): AIG cleanup removed %d nodes "
        "(this is a bug).\n",
        nNodes);
  // make sure everything is okay
  if (!Abc_NtkCheck(pNtkAig)) {
...
void Abc_NtkWriteAig(Abc_Ntk_t *pNtk, char *pFileName) {
  ...
  iLit = 0;
  Vec_IntWriteEntry(vId2Num,
                    2 * Abc_ObjId(Abc_AigConst1(pNtk)) + 1,
                    iLit++);
  Vec_IntWriteEntry(vId2Num,
                    2 * Abc_ObjId(Abc_AigConst1(pNtk)) + 0,
                    iLit++);
  Abc_NtkForEachPi(pNtk, pObj, i) {
    Vec_IntWriteEntry(vId2Num, 2 * Abc_ObjId(pObj) + 0,
                      iLit++);
    Vec_IntWriteEntry(vId2Num, 2 * Abc_ObjId(pObj) + 1,
                      iLit++);
  }
  ...
  fprintf(pFile, "NULL },\n");
  fprintf(pFile, "    { ");
  Abc_AigForEachAnd(pNtk, pObj, i) fprintf(
      pFile, "%d,",
      Vec_IntEntry(vId2Num, 2 * Abc_ObjFaninId0(pObj) +
                                Abc_ObjFaninC0(pObj)));
  fprintf(pFile, "0 },\n");
  fprintf(pFile, "    { ");
  Abc_AigForEachAnd(pNtk, pObj, i) fprintf(
      pFile, "%d,",
      Vec_IntEntry(vId2Num, 2 * Abc_ObjFaninId1(pObj) +
                                Abc_ObjFaninC1(pObj)));
  fprintf(pFile, "0 },\n");
  fprintf(pFile, "    { ");
  Abc_NtkForEachPo(pNtk, pObj, i) fprintf(
      pFile, "%d,",
      Vec_IntEntry(vId2Num, 2 * Abc_ObjFaninId0(pObj) +
                                Abc_ObjFaninC0(pObj)));
  fprintf(pFile, "0 },\n");
...
Abc_Ntk_t *Abc_NtkPutOnTop(Abc_Ntk_t *pNtk,
  ...
  Vec_PtrFree(vNodes);
  // transfer to the POs
  Abc_NtkForEachPi(pNtk2, pObj, i) pObj->pCopy =
      Abc_ObjChild0Copy(Abc_NtkPo(pNtk, i));
  // add internal nodes
