...
system.]
  ...
  Synopsis    [Procedures to derive the miter of two
circuits.]
  ...
  Revision    [$Id: abcMiter.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
static Abc_Ntk_t *Abc_NtkMiterInt(Abc_Ntk_t *pNtk1,
                                  Abc_Ntk_t *pNtk2,
                                  int fComb, int nPartSize,
                                  int fImplic, int fMulti);
static void Abc_NtkMiterPrepare(Abc_Ntk_t *pNtk1,
                                Abc_Ntk_t *pNtk2,
                                Abc_Ntk_t *pNtkMiter,
                                int fComb, int nPartSize,
                                int fMulti);
static void Abc_NtkMiterAddOne(Abc_Ntk_t *pNtk,
                               Abc_Ntk_t *pNtkMiter);
static void Abc_NtkMiterFinalize(Abc_Ntk_t *pNtk1,
                                 Abc_Ntk_t *pNtk2,
                                 Abc_Ntk_t *pNtkMiter,
                                 int fComb, int nPartSize,
                                 int fImplic, int fMulti);
static void Abc_NtkAddFrame(Abc_Ntk_t *pNetNew,
                            Abc_Ntk_t *pNet, int iFrame);

// to be exported
typedef void (*AddFrameMapping)(Abc_Obj_t *, Abc_Obj_t *,
                                int, void *);
extern Abc_Ntk_t *Abc_NtkFrames2(
    Abc_Ntk_t *pNtk, int nFrames, int fInitial,
    AddFrameMapping addFrameMapping, void *arg);
static void Abc_NtkAddFrame2(
    Abc_Ntk_t *pNtkFrames, Abc_Ntk_t *pNtk, int iFrame,
    Vec_Ptr_t *vNodes, AddFrameMapping addFrameMapping,
    void *arg);
...
/**Function*************************************************************
  ...
  Description [Preprocesses the networks to make sure that
they are strashed.]
...
Abc_Ntk_t *Abc_NtkMiter(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2,
  ...
  fRemove1 = (!Abc_NtkIsStrash(pNtk1) ||
              Abc_NtkGetChoiceNum(pNtk1)) &&
             (pNtk1 = Abc_NtkStrash(pNtk1, 0, 0, 0));
  fRemove2 = (!Abc_NtkIsStrash(pNtk2) ||
              Abc_NtkGetChoiceNum(pNtk2)) &&
...
/**Function*************************************************************
  ...
  Synopsis    [Derives the miter of two sequential
networks.]
...
Abc_Ntk_t *Abc_NtkMiterInt(Abc_Ntk_t *pNtk1,
                           Abc_Ntk_t *pNtk2, int fComb,
                           int nPartSize, int fImplic,
                           int fMulti) {
  char Buffer[1000];
  Abc_Ntk_t *pNtkMiter;
  ...
  sprintf(Buffer, "%s_%s_miter", pNtk1->pName,
          pNtk2->pName);
...
***********************************************************************/
void Abc_NtkMiterPrepare(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2,
                         Abc_Ntk_t *pNtkMiter, int fComb,
  ...
  if (fComb) {
    ...
    if (nPartSize <= 0) {
      ...
      if (fMulti) {
        Abc_NtkForEachCo(pNtk1, pObj, i) {
          ...
          Abc_ObjAssignName(pObjNew, "miter",
                            Abc_ObjName(pObjNew));
        }
    ...
    Abc_NtkForEachLatch(pNtk1, pObj, i) {
      ...
      Abc_ObjAssignName(Abc_ObjFanin0(pObjNew),
                        Abc_ObjName(Abc_ObjFanin0(pObj)),
                        "_1");
      Abc_ObjAssignName(Abc_ObjFanout0(pObjNew),
...
void Abc_NtkMiterAddOne(Abc_Ntk_t *pNtk,
  ...
  Abc_AigForEachAnd(pNtk, pNode, i) pNode->pCopy =
      Abc_AigAnd((Abc_Aig_t *)pNtkMiter->pManFunc,
                 Abc_ObjChild0Copy(pNode),
                 Abc_ObjChild1Copy(pNode));
}
...
void Abc_NtkMiterAddCone(Abc_Ntk_t *pNtk,
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pNode,
                      i) if (Abc_AigNodeIsAnd(pNode))
      pNode->pCopy = Abc_AigAnd(
      (Abc_Aig_t *)pNtkMiter->pManFunc,
      Abc_ObjChild0Copy(pNode), Abc_ObjChild1Copy(pNode));
  Vec_PtrFree(vNodes);
}

/**Function*************************************************************

  Synopsis    [Finalizes the miter by adding the output
part.]
...
void Abc_NtkMiterFinalize(Abc_Ntk_t *pNtk1,
  ...
  if (fComb) {
    ...
    Abc_NtkForEachCo(pNtk1, pNode, i) {
      if (fMulti) {
        pMiter = Abc_AigXor(
            (Abc_Aig_t *)pNtkMiter->pManFunc,
            Abc_ObjChild0Copy(pNode),
            Abc_ObjChild0Copy(Abc_NtkCo(pNtk2, i)));
    ...
    // connect new latches
    Abc_NtkForEachLatch(pNtk1, pNode, i) Abc_ObjAddFanin(
        Abc_ObjFanin0(pNode)->pCopy,
        Abc_ObjChild0Copy(Abc_ObjFanin0(pNode)));
    Abc_NtkForEachLatch(pNtk2, pNode, i) Abc_ObjAddFanin(
        Abc_ObjFanin0(pNode)->pCopy,
        Abc_ObjChild0Copy(Abc_ObjFanin0(pNode)));
  }
  // add the miter
  if (nPartSize <= 0) {
    if (!fMulti) {
      pMiter =
          Abc_AigMiter((Abc_Aig_t *)pNtkMiter->pManFunc,
                       vPairs, fImplic);
    ...
    // create partitions
    nParts = Abc_NtkCoNum(pNtk1) / nPartSize +
             (int)((Abc_NtkCoNum(pNtk1) % nPartSize) > 0);
    vPairsPart = Vec_PtrAlloc(nPartSize);
    for (i = 0; i < nParts; i++) {
      Vec_PtrClear(vPairsPart);
      for (k = 0; k < nPartSize; k++) {
        iCur = i * nPartSize + k;
        if (iCur >= Abc_NtkCoNum(pNtk1)) break;
        Vec_PtrPush(vPairsPart,
                    Vec_PtrEntry(vPairs, 2 * iCur));
        Vec_PtrPush(vPairsPart,
                    Vec_PtrEntry(vPairs, 2 * iCur + 1));
      }
      pMiter =
          Abc_AigMiter((Abc_Aig_t *)pNtkMiter->pManFunc,
                       vPairsPart, fImplic);
...
/**Function*************************************************************
  ...
  Description [The network should have the same names of
PIs.]
...
Abc_Ntk_t *Abc_NtkMiterAnd(Abc_Ntk_t *pNtk1,
  ...
  pOutput1 = Abc_ObjNotCond(Abc_ObjFanin0(pRoot1)->pCopy,
                            Abc_ObjFaninC0(pRoot1));
  pOutput2 =
      Abc_ObjNotCond(Abc_ObjFanin0(pRoot2)->pCopy,
                     (int)Abc_ObjFaninC0(pRoot2) ^ fCompl2);

  // create the miter of the two outputs
  if (fOr)
    pMiter = Abc_AigOr((Abc_Aig_t *)pNtkMiter->pManFunc,
                       pOutput1, pOutput2);
  else
    pMiter = Abc_AigAnd((Abc_Aig_t *)pNtkMiter->pManFunc,
                        pOutput1, pOutput2);
  Abc_ObjAddFanin(Abc_NtkPo(pNtkMiter, 0), pMiter);
...
/**Function*************************************************************
  ...
  Synopsis    [Derives the cofactor of the miter w.r.t. the
set of vars.]

  Description [The array of variable values contains -1/0/1
for each PI.
  -1 means this PI remains, 0/1 means this PI is set to
0/1.]
...
Abc_Ntk_t *Abc_NtkMiterCofactor(Abc_Ntk_t *pNtk,
  ...
  Vec_IntForEachEntry(vPiValues, Value, i) {
    if (Value == -1) continue;
    if (Value == 0) {
      Abc_NtkCi(pNtk, i)->pCopy =
          Abc_ObjNot(Abc_AigConst1(pNtkMiter));
      continue;
    }
    if (Value == 1) {
  ...
  // save the output
  pOutput1 = Abc_ObjNotCond(Abc_ObjFanin0(pRoot)->pCopy,
                            Abc_ObjFaninC0(pRoot));
  ...
  if (!Abc_NtkCheck(pNtkMiter)) {
    printf(
        "Abc_NtkMiterCofactor: The network check has "
        "failed.\n");
    Abc_NtkDelete(pNtkMiter);
    return NULL;
  }
...
/**Function*************************************************************
  ...
  Synopsis    [Derives the miter of two cofactors of one
output.]
...
Abc_Ntk_t *Abc_NtkMiterForCofactors(Abc_Ntk_t *pNtk,
  ...
  // start the new network
  pNtkMiter = Abc_NtkAlloc(ABC_NTK_STRASH, ABC_FUNC_AIG, 1);
  sprintf(Buffer, "%s_miter",
          Abc_ObjName(Abc_NtkCo(pNtk, Out)));
  pNtkMiter->pName = Extra_UtilStrsav(Buffer);
  ...
  // create the miter of the two outputs
  pMiter = Abc_AigXor((Abc_Aig_t *)pNtkMiter->pManFunc,
                      pOutput1, pOutput2);
  Abc_ObjAddFanin(Abc_NtkPo(pNtkMiter, 0), pMiter);
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkMiterQuantify(Abc_Ntk_t *pNtk, int In,
                                int fExist) {
  Abc_Ntk_t *pNtkMiter;
  Abc_Obj_t *pRoot, *pOutput1, *pOutput2, *pMiter;
  ...
  pNtkMiter = Abc_NtkAlloc(ABC_NTK_STRASH, ABC_FUNC_AIG, 1);
  pNtkMiter->pName =
      Extra_UtilStrsav(Abc_ObjName(Abc_NtkCo(pNtk, 0)));
  ...
  // save the output
  //    pOutput2 = Abc_ObjFanin0(pRoot)->pCopy;
  pOutput2 = Abc_ObjNotCond(Abc_ObjFanin0(pRoot)->pCopy,
                            Abc_ObjFaninC0(pRoot));
...
/**Function*************************************************************
  ...
  Synopsis    [Quantifies all the PIs existentially from the
only PO of the network.]
...
/**Function*************************************************************
  ...
  Description [Return 0 if the miter is sat for at least one
output.
  Return 1 if the miter is unsat for all its outputs.
Returns -1 if the
...
int Abc_NtkMiterIsConstant(Abc_Ntk_t *pMiter) {
  ...
  Abc_NtkForEachPo(pMiter, pNodePo, i) {
    ...
    if (Abc_AigNodeIsConst(pChild)) {
      assert(Abc_ObjRegular(pChild) ==
             Abc_AigConst1(pMiter));
    ...
    /*
            ...
            else if ( Abc_ObjRegular(pChild)->fPhase !=
       (unsigned)Abc_ObjIsComplement(pChild) )
            {
                return 0;
            }
    */
    // if the miter is undecided (or satisfiable), return
    // immediately
    else
      return -1;
  }
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkFrames(Abc_Ntk_t *pNtk, int nFrames,
  ...
  // create new latches (or their initial values) and
  // remember them in the new latches
  if (!fInitial) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) {
      ...
      if (Abc_LatchIsInitNone(pLatch) ||
          Abc_LatchIsInitDc(pLatch))  // don't-care initial
                                      // value - create a
                                      // new PI
      {
        pLatchOut->pCopy = Abc_NtkCreatePi(pNtkFrames);
        Abc_ObjAssignName(pLatchOut->pCopy,
                          Abc_ObjName(pLatchOut), NULL);
        Counter++;
      } else
        pLatchOut->pCopy =
            Abc_ObjNotCond(Abc_AigConst1(pNtkFrames),
                           Abc_LatchIsInit0(pLatch));
    }
    if (Counter)
      printf(
          "Warning: %d uninitialized latches are replaced "
          "by free PI variables.\n",
          Counter);
  }
  ...
  // connect the new latches to the outputs of the last
  // frame
  if (!fInitial) {
    // we cannot use pLatch->pCopy here because
    // pLatch->pCopy is used for temporary storage of
    // strashed values
    Abc_NtkForEachLatch(pNtk, pLatch, i)
        Abc_ObjAddFanin(Abc_ObjFanin0(pLatch)->pCopy,
                        Abc_ObjFanout0(pLatch)->pCopy);
  }
...
/**Function*************************************************************
  ...
  Description [Assumes that the latches of the old network
point
  to the outputs of the previous frame of the new network
(pLatch->pCopy).
  In the end, updates the latches of the old network to
point to the
...
***********************************************************************/
void Abc_NtkAddFrame(Abc_Ntk_t *pNtkFrames, Abc_Ntk_t *pNtk,
                     int iFrame) {
  int fVerbose = 0;
  ...
  Abc_NtkForEachPi(pNtk, pNode, i)
      Abc_ObjAssignName(Abc_NtkDupObj(pNtkFrames, pNode, 0),
                        Abc_ObjName(pNode), Buffer);
  // add the internal nodes
  Abc_AigForEachAnd(pNtk, pNode, i) pNode->pCopy =
      Abc_AigAnd((Abc_Aig_t *)pNtkFrames->pManFunc,
                 Abc_ObjChild0Copy(pNode),
                 Abc_ObjChild1Copy(pNode));
  // add the new POs
  ...
  // transfer the implementation of the latch inputs to the
  // latch outputs
  Abc_NtkForEachLatch(pNtk, pLatch, i) pLatch->pCopy =
      Abc_ObjChild0Copy(Abc_ObjFanin0(pLatch));
  Abc_NtkForEachLatch(pNtk, pLatch, i)
      Abc_ObjFanout0(pLatch)
          ->pCopy = pLatch->pCopy;
  // nodes after
  if (fVerbose)
    printf(
        "F = %4d : Total = %6d. Nodes = %6d. Prop = %s.\n",
        iFrame, Abc_NtkNodeNum(pNtk),
        Abc_NtkNodeNum(pNtkFrames) - NodeBef,
        Abc_AigNodeIsConst(
            Abc_ObjFanin0(Abc_NtkPo(pNtk, 0))->pCopy)
            ? "proof"
            : "unknown");
}
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkFrames2(Abc_Ntk_t *pNtk, int nFrames,
                          int fInitial,
  ...
  /*
      ...
      assert( Abc_NtkIsStrash(pNtk) );
      // start the new network
      pNtkFrames = Abc_NtkAlloc( ABC_NTK_STRASH,
     ABC_FUNC_AIG, 1 );
      sprintf( Buffer, "%s_%d_frames", pNtk->pName, nFrames
     );
      pNtkFrames->pName = Extra_UtilStrsav(Buffer);
     ...
     remember them in the new latches
      if ( !fInitial )
      {
          Abc_NtkForEachLatch( pNtk, pLatch, i ) {
              ...
              if (addFrameMapping)
     addFrameMapping(pLatch->pCopy, pLatch, 0, arg);
          }
      }
      else
      {
          ...
          Abc_NtkForEachLatch( pNtk, pLatch, i )
          {
              if ( Abc_LatchIsInitDc(pLatch) ) // don't-care
     initial value - create a new PI
              {
                  pLatch->pCopy =
     Abc_NtkCreatePi(pNtkFrames);
                  Abc_ObjAssignName( pLatch->pCopy,
     Abc_ObjName(pLatch), NULL );
                  Counter++;
              }
              else {
                  pLatch->pCopy = Abc_ObjNotCond(
     ...
     addFrameMapping(pLatch->pCopy, pLatch, 0, arg);
          ...
          if ( Counter )
              printf( "Warning: %d uninitialized latches are
     replaced by free PI variables.\n", Counter );
      }
      ...
      {
          Extra_ProgressBarUpdate( pProgress, i, NULL );
          Abc_NtkAddFrame2( pNtkFrames, pNtk, i, vNodes,
     addFrameMapping, arg );
      }
      Extra_ProgressBarStop( pProgress );
      Vec_PtrFree( vNodes );
     ...
     Abc_ObjName(pLatch), NULL );
      ...
      if ( !Abc_NtkCheck( pNtkFrames ) )
      {
          printf( "Abc_NtkFrames: The network check has
     failed.\n" );
          Abc_NtkDelete( pNtkFrames );
          return NULL;
      }
...
void Abc_NtkAddFrame2(Abc_Ntk_t *pNtkFrames,
  ...
  /*
      ...
      Abc_NtkForEachPi( pNtk, pNode, i )
      {
          ...
          Abc_ObjAssignName( pNodeNew, Abc_ObjName(pNode),
     Buffer );
          if (addFrameMapping) addFrameMapping(pNodeNew,
     pNode, iFrame, arg);
      }
      // add the internal nodes
     ...
     pNode, iFrame, arg);
      ...
      // transfer the implementation of the latch drivers to
     the latches

      // it is important that these two steps are performed
     it two loops
      ...
      {
          if (addFrameMapping) {
              // don't give Mike complemented pointers
     because he doesn't like it
              if (Abc_ObjIsComplement(pLatch->pCopy)) {
                  ...
                  pNodeNew->Level = 1 +
     Abc_ObjRegular(pLatch->pCopy)->Level;

                  pLatch->pNext = pNodeNew;
                  pLatch->pCopy = pNodeNew;
              }
              addFrameMapping(pLatch->pCopy, pLatch,
     iFrame+1, arg);
          }
      }
  */
...
/**Function*************************************************************
  ...
  Synopsis    [Splits the miter into two logic cones
combined by an EXOR]
...
int Abc_NtkDemiter(Abc_Ntk_t *pNtk) {
  ...
  if (!Abc_NodeIsExorType(
          Abc_ObjFanin0(Abc_NtkPo(pNtk, 0)))) {
    printf("The root of the miter is not an EXOR gate.\n");
    return 0;
  }
  pNodeC = Abc_NodeRecognizeMux(
      Abc_ObjFanin0(Abc_NtkPo(pNtk, 0)), &pNodeA, &pNodeB);
  ...
  printf(
      "First cone = %6d.  Second cone = %6d.  Common = "
      "%6d.\n",
...
int Abc_NtkCombinePos(Abc_Ntk_t *pNtk, int fAnd, int fXor) {
  ...
  if (Abc_NtkPoNum(pNtk) == 1) return 1;
  // start the result
  if (fAnd)
  ...
  Abc_NtkForEachPo(pNtk, pNode, i) if (fAnd) pMiter =
      Abc_AigAnd((Abc_Aig_t *)pNtk->pManFunc, pMiter,
                 Abc_ObjChild0(pNode));
  else if (fXor) pMiter =
      Abc_AigXor((Abc_Aig_t *)pNtk->pManFunc, pMiter,
                 Abc_ObjChild0(pNode));
  else pMiter = Abc_AigOr((Abc_Aig_t *)pNtk->pManFunc,
