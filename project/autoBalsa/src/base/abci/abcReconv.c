...
system.]
  ...
  Revision    [$Id: abcReconv.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
struct Abc_ManCut_t_ {
  ...
  int nNodeSizeMax;  // the limit on the size of the
                     // supernode
  int nConeSizeMax;  // the limit on the size of the
                     // containing cone
  int nNodeFanStop;  // the limit on the size of the
                     // supernode
  int nConeFanStop;  // the limit on the size of the
                     // containing cone
  // internal parameters
  Vec_Ptr_t *vNodeLeaves;  // fanins of the collapsed node
                           // (the cut)
  Vec_Ptr_t *vConeLeaves;  // fanins of the containing cone
  Vec_Ptr_t *vVisited;     // the visited nodes
  Vec_Vec_t
      *vLevels;  // the data structure to compute TFO nodes
  Vec_Ptr_t *vNodesTfo;  // the nodes in the TFO of the cut
};

static int Abc_NodeBuildCutLevelOne_int(Vec_Ptr_t *vVisited,
                                        Vec_Ptr_t *vLeaves,
                                        int nSizeLimit,
                                        int nFaninLimit);
static int Abc_NodeBuildCutLevelTwo_int(Vec_Ptr_t *vVisited,
                                        Vec_Ptr_t *vLeaves,
                                        int nFaninLimit);
static void Abc_NodeConeMarkCollect_rec(
    Abc_Obj_t *pNode, Vec_Ptr_t *vVisited);
...
/**Function*************************************************************
  ...
  Synopsis    [Evaluate the cost of removing the node from
the set of leaves.]

  Description [Returns the number of new leaves that will be
brought in.
  Returns large number if the node cannot be removed from
the set of leaves.]
...
***********************************************************************/
static inline int Abc_NodeGetLeafCostOne(Abc_Obj_t *pNode,
                                         int nFaninLimit) {
  int Cost;
  // make sure the node is in the construction zone
  assert(pNode->fMarkB == 1);
  // cannot expand over the PI node
  if (Abc_ObjIsCi(pNode)) return 999;
  // get the cost of the cone
  Cost = (!Abc_ObjFanin0(pNode)->fMarkB) +
         (!Abc_ObjFanin1(pNode)->fMarkB);
  // always accept if the number of leaves does not increase
  if (Cost < 2) return Cost;
  // skip nodes with many fanouts
  if (Abc_ObjFanoutNum(pNode) > nFaninLimit) return 999;
  // return the number of nodes that will be on the leaves
...
***********************************************************************/
static inline int Abc_NodeGetLeafCostTwo(
    Abc_Obj_t *pNode, int nFaninLimit,
    Abc_Obj_t **ppLeafToAdd, Abc_Obj_t **pNodeToMark1,
  ...
  // count the number of unique grandchildren that will be
  // included
  // return infinite cost if this number if more than 1
  if (Abc_ObjIsCi(pFanin0) && Abc_ObjIsCi(pFanin1))
    return 999;
  // consider the special case when a non-CI fanin can be
  // dropped
  if (!Abc_ObjIsCi(pFanin0) &&
      Abc_ObjFanin0(pFanin0)->fMarkB &&
      Abc_ObjFanin1(pFanin0)->fMarkB) {
    *ppLeafToAdd = pFanin1;
  ...
  if (!Abc_ObjIsCi(pFanin1) &&
      Abc_ObjFanin0(pFanin1)->fMarkB &&
      Abc_ObjFanin1(pFanin1)->fMarkB) {
    *ppLeafToAdd = pFanin0;
  ...
  if (Abc_ObjIsCi(pFanin0)) {
    ...
    *pNodeToMark1 = pFanin0;
    ...
    if (!pGrand->fMarkB) {
      if (pGrandToAdd && pGrandToAdd != pGrand) return 999;
      pGrandToAdd = pGrand;
    }
...
/**Function*************************************************************
  ...
  Synopsis    [Finds a fanin-limited, reconvergence-driven
cut for the node.]
...
Vec_Ptr_t *Abc_NodeFindCut(Abc_ManCut_t *p,
  ...
  // compute the cut
  while (Abc_NodeBuildCutLevelOne_int(
      p->vVisited, p->vNodeLeaves, p->nNodeSizeMax,
      p->nNodeFanStop))
    ;
  ...
  // compute the containing cut
  while (Abc_NodeBuildCutLevelOne_int(
      p->vVisited, p->vConeLeaves, p->nConeSizeMax,
      p->nConeFanStop))
    ;
...
/**Function*************************************************************
  ...
  Synopsis    [Builds reconvergence-driven cut by changing
one leaf at a time.]

  Description [This procedure looks at the current leaves
and tries to change
  one leaf at a time in such a way that the cut grows as
little as possible.
  In evaluating the fanins, this procedure looks only at
their immediate
  predecessors (this is why it is called a one-level
...
int Abc_NodeBuildCutLevelOne_int(Vec_Ptr_t *vVisited,
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vLeaves, pNode, i) {
    ...
    //        if ( CostBest > CostCur ) // performance
    //        improvement: expand the variable with the
    //        smallest level
    if (CostBest > CostCur ||
        (CostBest == CostCur &&
         pNode->Level > pFaninBest->Level)) {
      CostBest = CostCur;
      pFaninBest = pNode;
    }
    if (CostBest == 0) break;
  }
  if (pFaninBest == NULL) return 0;
  //        return Abc_NodeBuildCutLevelTwo_int( vVisited,
  //        vLeaves, nFaninLimit );

  assert(CostBest < 3);
  if (vLeaves->nSize - 1 + CostBest > nSizeLimit) return 0;
  //        return Abc_NodeBuildCutLevelTwo_int( vVisited,
  //        vLeaves, nFaninLimit );
...
and tries to change
  one leaf at a time in such a way that the cut grows as
little as possible.
  In evaluating the fanins, this procedure looks across two
levels of fanins
  (this is why it is called a two-level construction
procedure).]
...
int Abc_NodeBuildCutLevelTwo_int(Vec_Ptr_t *vVisited,
                                 Vec_Ptr_t *vLeaves,
                                 int nFaninLimit) {
  Abc_Obj_t *pNode = NULL, *pLeafToAdd, *pNodeToMark1,
            *pNodeToMark2;
  int CostCur = 0, i;
  // find the best fanin
  Vec_PtrForEachEntry(Abc_Obj_t *, vLeaves, pNode, i) {
    CostCur = Abc_NodeGetLeafCostTwo(
        pNode, nFaninLimit, &pLeafToAdd, &pNodeToMark1,
        &pNodeToMark2);
    if (CostCur < 2) break;
  }
  if (CostCur > 2) return 0;
  // remove the node from the array
...
***********************************************************************/
void Abc_NodeConeCollect(Abc_Obj_t **ppRoots, int nRoots,
                         Vec_Ptr_t *vLeaves,
                         Vec_Ptr_t *vVisited,
                         int fIncludeFanins) {
  Abc_Obj_t *pTemp;
...
void Abc_NodeConeMarkCollect_rec(Abc_Obj_t *pNode,
  ...
  if (Abc_ObjIsNode(pNode)) {
    Abc_NodeConeMarkCollect_rec(Abc_ObjFanin0(pNode),
                                vVisited);
    Abc_NodeConeMarkCollect_rec(Abc_ObjFanin1(pNode),
                                vVisited);
  }
...
/**Function*************************************************************
  ...
  Synopsis    [Returns BDD representing the logic function
of the cone.]
...
***********************************************************************/
DdNode *Abc_NodeConeBdd(DdManager *dd, DdNode **pbVars,
                        Abc_Obj_t *pRoot,
                        Vec_Ptr_t *vLeaves,
                        Vec_Ptr_t *vVisited) {
  Abc_Obj_t *pNode;
  ...
  int i;
  // get the nodes in the cut without fanins in the DFS
  // order
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vVisited, pNode, i) {
    ...
    bFunc0 = Cudd_NotCond(Abc_ObjFanin0(pNode)->pCopy,
                          (int)Abc_ObjFaninC0(pNode));
    bFunc1 = Cudd_NotCond(Abc_ObjFanin1(pNode)->pCopy,
...
/**Function*************************************************************
  ...
  Synopsis    [Returns BDD representing the transition
relation of the cone.]
...
***********************************************************************/
DdNode *Abc_NodeConeDcs(DdManager *dd, DdNode **pbVarsX,
                        DdNode **pbVarsY,
                        Vec_Ptr_t *vLeaves,
                        Vec_Ptr_t *vRoots,
                        Vec_Ptr_t *vVisited) {
  DdNode *bFunc0, *bFunc1, *bFunc, *bTrans, *bTemp, *bCube,
      *bResult;
  Abc_Obj_t *pNode;
  int i;
  // get the nodes in the cut without fanins in the DFS
  // order
  Abc_NodeConeCollect((Abc_Obj_t **)vRoots->pArray,
                      vRoots->nSize, vLeaves, vVisited, 0);
  // set the elementary BDDs
  Vec_PtrForEachEntry(Abc_Obj_t *, vLeaves, pNode, i)
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vRoots, pNode, i) {
    bFunc = Cudd_bddXnor(dd, (DdNode *)pNode->pCopy,
                         pbVarsY[i]);
    Cudd_Ref(bFunc);
    bTrans = Cudd_bddAnd(dd, bTemp = bTrans, bFunc);
    Cudd_Ref(bTrans);
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vVisited, pNode, i)
      Cudd_RecursiveDeref(dd, (DdNode *)pNode->pCopy);
  // compute don't-cares
  bCube = Extra_bddComputeRangeCube(
      dd, vRoots->nSize, vRoots->nSize + vLeaves->nSize);
  Cudd_Ref(bCube);
  bResult = Cudd_bddExistAbstract(dd, bTrans, bCube);
  Cudd_Ref(bResult);
...
/**Function*************************************************************
  ...
  Synopsis    [Collects the TFO of the cut in the
topological order.]

  Description [TFO of the cut is defined as a set of nodes,
for which the cut
  is a cut, that is, every path from the collected nodes to
the CIs goes through
  a node in the cut. The nodes are collected if their level
does not exceed
  the given number (LevelMax). The nodes are returned in the
topological order.
  If the root node is given, its MFFC is marked, so that the
collected nodes
...
Vec_Ptr_t *Abc_NodeCollectTfoCands(Abc_ManCut_t *p,
  ...
  LevelMin = -1;
  Vec_PtrForEachEntry(Abc_Obj_t *, vLeaves, pNode, i) {
    if (pNode->Level > (unsigned)LevelMax) continue;
  ...
  // mark MFFC
  if (pRoot) Abc_NodeMffcLabelAig(pRoot);

  // go through the levels up
  Vec_PtrClear(p->vNodesTfo);
  Vec_VecForEachEntryStart(Abc_Obj_t *, p->vLevels, pNode,
                           i, k, LevelMin) {
    if (i > LevelMax) break;
