...
system.]
  ...
  Revision    [$Id: abcIvy.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
// extern int Fra_FraigSat( Aig_Man_t * pMan, ABC_INT64_T
// nConfLimit, ABC_INT64_T nInsLimit, int fFlipBits, int
// fAndOuts, int fNewSolver, int fVerbose );
extern Ivy_Obj_t *Dec_GraphToNetworkIvy(
    Ivy_Man_t *pMan, Dec_Graph_t *pGraph);
...
static Abc_Ntk_t *Abc_NtkFromIvy(Abc_Ntk_t *pNtkOld,
                                 Ivy_Man_t *pMan);
static Abc_Ntk_t *Abc_NtkFromIvySeq(Abc_Ntk_t *pNtkOld,
                                    Ivy_Man_t *pMan,
                                    int fHaig);
static Ivy_Man_t *Abc_NtkToIvy(Abc_Ntk_t *pNtkOld);

static void Abc_NtkStrashPerformAig(Abc_Ntk_t *pNtk,
                                    Ivy_Man_t *pMan);
static Ivy_Obj_t *Abc_NodeStrashAig(Ivy_Man_t *pMan,
                                    Abc_Obj_t *pNode);
static Ivy_Obj_t *Abc_NodeStrashAigSopAig(Ivy_Man_t *pMan,
                                          Abc_Obj_t *pNode,
                                          char *pSop);
static Ivy_Obj_t *Abc_NodeStrashAigExorAig(Ivy_Man_t *pMan,
                                           Abc_Obj_t *pNode,
                                           char *pSop);
static Ivy_Obj_t *Abc_NodeStrashAigFactorAig(
    Ivy_Man_t *pMan, Abc_Obj_t *pNode, char *pSop);

typedef int Abc_Edge_t;
static inline Abc_Edge_t Abc_EdgeCreate(int Id,
                                        int fCompl) {
  return (Id << 1) | fCompl;
}
static inline int Abc_EdgeId(Abc_Edge_t Edge) {
  return Edge >> 1;
}
static inline int Abc_EdgeIsComplement(Abc_Edge_t Edge) {
  return Edge & 1;
}
static inline Abc_Edge_t Abc_EdgeRegular(Abc_Edge_t Edge) {
  return (Edge >> 1) << 1;
}
static inline Abc_Edge_t Abc_EdgeNot(Abc_Edge_t Edge) {
  return Edge ^ 1;
}
static inline Abc_Edge_t Abc_EdgeNotCond(Abc_Edge_t Edge,
                                         int fCond) {
  return Edge ^ fCond;
}
static inline Abc_Edge_t Abc_EdgeFromNode(
    Abc_Obj_t *pNode) {
  return Abc_EdgeCreate(Abc_ObjRegular(pNode)->Id,
                        Abc_ObjIsComplement(pNode));
}
static inline Abc_Obj_t *Abc_EdgeToNode(Abc_Ntk_t *p,
                                        Abc_Edge_t Edge) {
  return Abc_ObjNotCond(Abc_NtkObj(p, Abc_EdgeId(Edge)),
                        Abc_EdgeIsComplement(Edge));
}

static inline Abc_Obj_t *Abc_ObjFanin0Ivy(Abc_Ntk_t *p,
                                          Ivy_Obj_t *pObj) {
  return Abc_ObjNotCond(
      Abc_EdgeToNode(p, Ivy_ObjFanin0(pObj)->TravId),
      Ivy_ObjFaninC0(pObj));
}
static inline Abc_Obj_t *Abc_ObjFanin1Ivy(Abc_Ntk_t *p,
                                          Ivy_Obj_t *pObj) {
  return Abc_ObjNotCond(
      Abc_EdgeToNode(p, Ivy_ObjFanin1(pObj)->TravId),
      Ivy_ObjFaninC1(pObj));
}

static Vec_Int_t *Abc_NtkCollectLatchValuesIvy(
    Abc_Ntk_t *pNtk, int fUseDcs);
...
Ivy_Man_t *Abc_NtkIvyBefore(Abc_Ntk_t *pNtk, int fSeq,
  ...
  if (Abc_NtkIsBddLogic(pNtk)) {
    if (!Abc_NtkBddToSop(pNtk, -1, ABC_INFINITY)) {
      printf(
          "Abc_NtkIvyBefore(): Converting to SOPs has "
          "failed.\n");
      return NULL;
    }
  }
  if (fSeq && Abc_NtkCountSelfFeedLatches(pNtk)) {
    printf(
        "Warning: The network has %d self-feeding "
        "latches.\n",
  ...
  if (Abc_NtkGetChoiceNum(pNtk))
    printf(
        "Warning: The choice nodes in the initial AIG are "
        "removed by strashing.\n");
  ...
  if (fSeq) {
    ...
    Vec_Int_t *vInit =
        Abc_NtkCollectLatchValuesIvy(pNtk, fUseDc);
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkIvyAfter(Abc_Ntk_t *pNtk, Ivy_Man_t *pMan,
                           int fSeq, int fHaig) {
  Abc_Ntk_t *pNtkAig;
  ...
  if (!fHaig && fCleanup &&
    ...
    printf(
        "Warning: AIG cleanup removed %d nodes (this is "
        "not a bug).\n",
        nNodes);
  // duplicate EXDC
  if (pNtk->pExdc) pNtkAig->pExdc = Abc_NtkDup(pNtk->pExdc);
...
/**Function*************************************************************
  ...
  Synopsis    [Gives the current ABC network to AIG manager
for processing.]
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkIvyHaig(Abc_Ntk_t *pNtk, int nIters,
                          int fUseZeroCost, int fVerbose) {
  Abc_Ntk_t *pNtkAig;
  Ivy_Man_t *pMan;
  abctime clk;
  //    int i;
...
Abc_Ntk_t *Abc_NtkIvyRewrite(Abc_Ntk_t *pNtk,
  ...
  if (pMan == NULL) return NULL;
  // timeRetime = Abc_Clock();
  Ivy_ManRewritePre(pMan, fUpdateLevel, fUseZeroCost,
                    fVerbose);
  // timeRetime = Abc_Clock() - timeRetime;
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkIvySat(Abc_Ntk_t *pNtk, int nConfLimit,
                         int fVerbose) {
...
/**Function*************************************************************
  ...
  Synopsis    [Sets the final nodes to point to the original
nodes.]
...
void Abc_NtkTransferPointers(Abc_Ntk_t *pNtk,
  ...
  Abc_NtkForEachCi(pNtk, pObj, i) pObj->pCopy =
      Abc_NtkCi(pNtkAig, i);
  Abc_NtkForEachCo(pNtk, pObj, i) pObj->pCopy =
      Abc_NtkCo(pNtkAig, i);
  Abc_NtkForEachLatch(pNtk, pObj, i) pObj->pCopy =
      Abc_NtkBox(pNtkAig, i);
  Abc_NtkForEachNode(pNtk, pObj, i) {
    pObjIvy = (Ivy_Obj_t *)pObj->pCopy;
    if (pObjIvy == NULL) continue;
    pObjFraig = Ivy_ObjEquiv(pObjIvy);
    if (pObjFraig == NULL) continue;
    pObj->pCopy = Abc_EdgeToNode(
        pNtkAig, Ivy_Regular(pObjFraig)->TravId);
    pObj->pCopy = Abc_ObjNotCond(
        pObj->pCopy, Ivy_IsComplement(pObjFraig));
  }
}
...
***********************************************************************/
Abc_Ntk_t *Abc_NtkIvyFraig(Abc_Ntk_t *pNtk, int nConfLimit,
                           int fDoSparse, int fProve,
                           int fTransfer, int fVerbose) {
  Ivy_FraigParams_t Params, *pParams = &Params;
  Abc_Ntk_t *pNtkAig;
  Ivy_Man_t *pMan, *pTemp;
  pMan = Abc_NtkIvyBefore(pNtk, 0, 0);
...
int Abc_NtkIvyProve(Abc_Ntk_t **ppNtk, void *pPars) {
  ...
  // check the case when the 0000 simulation pattern detect
  // the bug
  pObj = Abc_NtkPo(pNtk, 0);
  pFanin = Abc_ObjFanin0(pObj);
  if (Abc_ObjFanin0(pObj)->fPhase !=
  ...
  //    pParams->nMiteringLimitStart  = 300;    // starting
  //    mitering limit
  // to be
  //    pParams->nMiteringLimitStart  = 5000;    // starting
  //    mitering limit

  // if SAT only, solve without iteration
  //    RetValue = Abc_NtkMiterSat( pNtk,
  //    2*(ABC_INT64_T)pParams->nMiteringLimitStart,
  //    (ABC_INT64_T)0, 0, NULL, NULL );
  pMan2 = Abc_NtkToDar(pNtk, 0, 0);
  RetValue = Fra_FraigSat(
      pMan2, (ABC_INT64_T)pParams->nMiteringLimitStart,
      (ABC_INT64_T)0, 0, 0, 0, 1, 0, 0, 0);
  ...
  // try to prove it using brute force SAT with good CNF
  // encoding
  if (RetValue < 0) {
    pMan2 = Abc_NtkToDar(pNtk, 0, 0);
    ...
    if (pParams->fVerbose) {
      ...
      printf(
          "Intermediate reduced miter is written into file "
          "\"%s\".\n",
          pFileName);
    }
    RetValue =
        Fra_FraigSat(pMan2, pParams->nMiteringLimitLast, 0,
                     0, 0, 0, 0, 0, 0, pParams->fVerbose);
    pNtk->pModel = (int *)pMan2->pData, pMan2->pData = NULL;
    Aig_ManStop(pMan2);
  }
...
// try to prove it using brute force BDDs
  ...
  if (RetValue < 0 && pParams->fUseBdds) {
    ...
    pNtk = Abc_NtkCollapse(pNtkTemp = pNtk,
                           pParams->nBddSizeLimit, 0,
                           pParams->fBddReorder, 0);
    if (pNtk) {
      Abc_NtkDelete(pNtkTemp);
      RetValue =
          ((Abc_NtkNodeNum(pNtk) == 1) &&
           (Abc_ObjFanin0(Abc_NtkPo(pNtk, 0))->pData ==
            Cudd_ReadLogicZero(
                (DdManager *)pNtk->pManFunc)));
    } else
      pNtk = pNtkTemp;
  }
#endif
...
Abc_Ntk_t *Abc_NtkIvy(Abc_Ntk_t *pNtk) {
  ...
  if (Abc_NtkCountSelfFeedLatches(pNtk)) {
    printf(
        "Warning: The network has %d self-feeding latches. "
        "Quitting.\n",
        Abc_NtkCountSelfFeedLatches(pNtk));
    return NULL;
  }
  ...
  Abc_AigCleanup(pNtkAig->pManFunc)) )
          printf( "Warning: AIG cleanup removed %d nodes
  (this is not a bug).\n", nNodes );
      // duplicate EXDC
      if ( pNtk->pExdc )
      ...
      {
          ABC_FREE( pInit );
          printf( "Abc_NtkStrash: The network check has
  failed.\n" );
          Abc_NtkDelete( pNtkAig );
          return NULL;
      }
...
/**Function*************************************************************
  ...
  Synopsis    [Converts the network from the AIG manager
into ABC.]
...
Abc_Ntk_t *Abc_NtkFromIvy(Abc_Ntk_t *pNtkOld,
  ...
  Vec_Int_t *vNodes;
  Abc_Ntk_t *pNtk;
  Abc_Obj_t *pObj, *pObjNew, *pFaninNew, *pFaninNew0,
      *pFaninNew1;
  Ivy_Obj_t *pNode;
  int i;
  // perform strashing
  pNtk = Abc_NtkStartFrom(pNtkOld, ABC_NTK_STRASH,
                          ABC_FUNC_AIG);
  // transfer the pointers to the basic nodes
  Ivy_ManConst1(pMan)->TravId =
      Abc_EdgeFromNode(Abc_AigConst1(pNtk));
  Abc_NtkForEachCi(pNtkOld, pObj, i) Ivy_ManPi(pMan, i)
      ->TravId = Abc_EdgeFromNode(pObj->pCopy);
  // rebuild the AIG
  ...
  Ivy_ManForEachNodeVec(pMan, vNodes, pNode, i) {
    ...
    if (Ivy_ObjIsExor(pNode))
      pObjNew = Abc_AigXor((Abc_Aig_t *)pNtk->pManFunc,
                           pFaninNew0, pFaninNew1);
    else
      pObjNew = Abc_AigAnd((Abc_Aig_t *)pNtk->pManFunc,
                           pFaninNew0, pFaninNew1);
...
Abc_Ntk_t *Abc_NtkFromIvySeq(Abc_Ntk_t *pNtkOld,
  ...
  // perform strashing
  pNtk = Abc_NtkStartFromNoLatches(pNtkOld, ABC_NTK_STRASH,
                                   ABC_FUNC_AIG);
  // transfer the pointers to the basic nodes
  Ivy_ManConst1(pMan)->TravId =
      Abc_EdgeFromNode(Abc_AigConst1(pNtk));
  Abc_NtkForEachPi(pNtkOld, pObj, i) Ivy_ManPi(pMan, i)
      ->TravId = Abc_EdgeFromNode(pObj->pCopy);
  ...
  Ivy_ManForEachNodeVec(pMan, vNodes, pNode, i) {
    ...
    if (fHaig && pNode->pEquiv && Ivy_ObjRefs(pNode) > 0) {
      ...
      for (pTemp = pNode->pEquiv; pTemp != pNode;
           pTemp = Ivy_Regular(pTemp->pEquiv)) {
        pFaninNew1 = Abc_EdgeToNode(pNtk, pTemp->TravId);
        //                pFaninNew1->fPhase =
        //                Ivy_IsComplement( pTemp->pEquiv );
  ...
  Ivy_ManForEachNodeVec(pMan, vLatches, pNode, i) {
    ...
    Abc_ObjAddFanin(Abc_ObjFanin0(Abc_NtkBox(pNtk, i)),
                    pFaninNew);
  }
...
Ivy_Man_t *Abc_NtkToIvy(Abc_Ntk_t *pNtkOld) {
  ...
  int i;
  // create the manager
  assert(Abc_NtkHasSop(pNtkOld) ||
         Abc_NtkIsStrash(pNtkOld));
  pMan = Ivy_ManStart();
  // create the PIs
  if (Abc_NtkIsStrash(pNtkOld))
    Abc_AigConst1(pNtkOld)->pCopy =
        (Abc_Obj_t *)Ivy_ManConst1(pMan);
  Abc_NtkForEachCi(pNtkOld, pObj, i) pObj->pCopy =
      (Abc_Obj_t *)Ivy_ObjCreatePi(pMan);
...
void Abc_NtkStrashPerformAig(Abc_Ntk_t *pNtk,
  ...
  Vec_PtrForEachEntry(Abc_Obj_t *, vNodes, pNode, i) {
    ...
    pNode->pCopy =
        (Abc_Obj_t *)Abc_NodeStrashAig(pMan, pNode);
  }
...
Ivy_Obj_t *Abc_NodeStrashAig(Ivy_Man_t *pMan,
  ...
  if (Abc_NodeIsConst(pNode))
    return Ivy_NotCond(Ivy_ManConst1(pMan),
                       Abc_SopIsConst0(pSop));

  // decide when to use factoring
  if (fUseFactor && Abc_ObjFaninNum(pNode) > 2 &&
      Abc_SopGetCubeNum(pSop) > 1 &&
      !Abc_SopIsExorType(pSop))
...
Ivy_Obj_t *Abc_NodeStrashAigSopAig(Ivy_Man_t *pMan,
  ...
  Abc_SopForEachCube(pSop, nFanins, pCube) {
    ...
    Abc_ObjForEachFanin(pNode, pFanin,
                        i)  // pFanin can be a net
    {
      if (pCube[i] == '1')
        pAnd =
            Ivy_And(pMan, pAnd, (Ivy_Obj_t *)pFanin->pCopy);
      else if (pCube[i] == '0')
        pAnd = Ivy_And(pMan, pAnd,
                       Ivy_Not((Ivy_Obj_t *)pFanin->pCopy));
    }
  ...
  if (Abc_SopIsComplement(pSop)) pSum = Ivy_Not(pSum);
  return pSum;
}
...
Ivy_Obj_t *Abc_NodeStrashAigFactorAig(Ivy_Man_t *pMan,
  ...
  //    extern Ivy_Obj_t * Dec_GraphToNetworkAig( Ivy_Man_t
  //    * pMan, Dec_Graph_t * pGraph );
  ...
  // collect the fanins
  Dec_GraphForEachLeaf(pFForm, pNode, i) pNode->pFunc =
      Abc_ObjFanin(pRoot, i)->pCopy;
  // perform strashing
