...
Abc_Obj_t *isSameFanin(Abc_Obj_t *pA, Abc_Obj_t *pB, Abc_Obj_t *pC,
                       Abc_Obj_t *pD) {
  bool pC_same = pC == pA || pC == pB;
  bool pD_same = pD == pA || pD == pB;
  if (pC_same == pD_same) return (Abc_Obj_t *)NULL;
  if (pC_same) {
    if (pC == pA)
      return pA;
    else
...
Abc_Obj_t *commonInput(Abc_Obj_t *a, Abc_Obj_t *b) {
  ...
  if (input_a0 == input_b0 || input_a0 == input_b1) return input_a0;

  if (input_a1 == input_b0 || input_a1 == input_b1) return input_a1;
...
bool checkCase1(Abc_Obj_t *G9) {
  ...
  if (Abc_ObjIsComplement(G8) == Abc_ObjIsComplement(G5)) return false;
  if (isConstOrPi(G8) || isConstOrPi(G5)) return false;
  if (Abc_ObjIsComplement(G8)) swap(G8, G5);

  G8 = Abc_ObjRegular(G8);
  G5 = Abc_ObjRegular(G5);
  Abc_Obj_t *G6 = Abc_ObjChild0(G8);
  Abc_Obj_t *G7 = Abc_ObjChild1(G8);
  if (!Abc_ObjIsComplement(G6) || !Abc_ObjIsComplement(G7)) return false;
  ...
  if (succ) {
    ...
    cout << Abc_ObjId(G9) << " = MAJ("
         << (Abc_ObjIsComplement(a) ? string("-") : string(""))
         << Abc_ObjId(aR) << ", "
         << (Abc_ObjIsComplement(b) ? string("-") : string(""))
         << Abc_ObjId(bR) << ", "
         << (Abc_ObjIsComplement(c) ? string("-") : string(""))
         << Abc_ObjId(cR) << ")\n";
    // 25 = MAJ(5, −14, 22)
  }
  return succ;
...
bool same2InputButInverse(Abc_Obj_t *i, Abc_Obj_t *_i) {
  if (isConstOrPi(i) || isConstOrPi(_i)) return false;
...
bool checkCase2_G6G7(Abc_Obj_t *G6, Abc_Obj_t *G7, Abc_Obj_t *G8) {
  if (isConstOrPi(G6) || isConstOrPi(G6)) return false;
  Abc_Obj_t *in0, *in1;
...
bool checkCase2(Abc_Obj_t *G8) {
  ...
  if (!Abc_ObjIsComplement(G7) || !Abc_ObjIsComplement(G6)) return false;

  G7 = Abc_ObjRegular(G7);
  G6 = Abc_ObjRegular(G6);

  if (isConstOrPi(G7) || isConstOrPi(G6)) return false;

  int r = 0;
  if (Abc_ObjIsComplement(Abc_ObjChild0(G6)) ||
      Abc_ObjIsComplement(Abc_ObjChild1(G6)))
    r += checkCase2_G6G7(G6, G7, G8);

  if (Abc_ObjIsComplement(Abc_ObjChild0(G7)) ||
      Abc_ObjIsComplement(Abc_ObjChild1(G7)))
