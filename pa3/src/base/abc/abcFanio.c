...
***********************************************************************/
static inline void Vec_IntPushMem(Mem_Step_t *pMemMan, Vec_Int_t *p,
                                  int Entry) {
  if (p->nSize == p->nCap) {
    int *pArray;
    int i;

    if (p->nSize == 0) p->nCap = 1;
    if (pMemMan)
      pArray = (int *)Mem_StepEntryFetch(pMemMan, p->nCap * 8);
    else
      pArray = ABC_ALLOC(int, p->nCap * 2);
    if (p->pArray) {
      for (i = 0; i < p->nSize; i++) pArray[i] = p->pArray[i];
      if (pMemMan)
...
void Abc_ObjAddFanin(Abc_Obj_t *pObj, Abc_Obj_t *pFanin) {
  ...
  assert(!Abc_ObjIsPi(pObj) &&
         !Abc_ObjIsPo(pFaninR));  // fanin of PI or fanout of PO
  assert(!Abc_ObjIsCo(pObj) ||
         !Abc_ObjFaninNum(pObj));  // CO with two fanins
  assert(!Abc_ObjIsNet(pObj) ||
         !Abc_ObjFaninNum(pObj));  // net with two fanins
...
/**Function*************************************************************
  ...
  Description [The node is pObj. An old fanin of this node (pFaninOld) has
to be
  replaced by a new fanin (pFaninNew). Assumes that the node and the old
fanin
  are not complemented. The new fanin can be complemented. In this case,
the
  polarity of the new fanin will change, compared to the polarity of the
old fanin.]
...
***********************************************************************/
void Abc_ObjPatchFanin(Abc_Obj_t *pObj, Abc_Obj_t *pFaninOld,
                       Abc_Obj_t *pFaninNew) {
  ...
  // replace the old fanin entry by the new fanin entry (removes
  // attributes)
  ...
  if (Abc_ObjIsComplement(pFaninNew)) Abc_ObjXorFaninC(pObj, iFanin);

  //    if ( Abc_NtkIsSeq(pObj->pNtk) && (nLats = Seq_ObjFaninL(pObj,
  //    iFanin)) )
...
/**Function*************************************************************
  ...
  Synopsis    [Replaces pObj by iObjNew in the fanin arrays of the
fanouts.]
...
void Abc_ObjPatchFanoutFanin(Abc_Obj_t *pObj, int iObjNew) {
  ...
  Abc_ObjForEachFanout(pObj, pFanout, i) {
    Vec_IntForEachEntry(&pFanout->vFanins, Entry,
                        k) if (Entry == (int)Abc_ObjId(pObj)) {
...
/**Function*************************************************************
  ...
  Synopsis    [Inserts one-input node of the type specified between the
nodes.]
...
***********************************************************************/
Abc_Obj_t *Abc_ObjInsertBetween(Abc_Obj_t *pNodeIn, Abc_Obj_t *pNodeOut,
                                Abc_ObjType_t Type) {
  Abc_Obj_t *pNodeNew;
  int iFanoutIndex, iFaninIndex;
  // find pNodeOut among the fanouts of pNodeIn
  if ((iFanoutIndex = Vec_IntFind(&pNodeIn->vFanouts, pNodeOut->Id)) ==
      -1) {
  ...
  Vec_IntPushMem(pNodeNew->pNtk->pMmStep, &pNodeNew->vFanouts,
                 pNodeOut->Id);
...
void Abc_ObjTransferFanout(Abc_Obj_t *pNodeFrom, Abc_Obj_t *pNodeTo) {
  ...
  for (i = 0; i < vFanouts->nSize; i++)
    Abc_ObjPatchFanin((Abc_Obj_t *)vFanouts->pArray[i], pNodeFrom,
                      pNodeTo);
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the index of the fanin in the fanin list of the
fanout.]
...
int Abc_ObjFanoutFaninNum(Abc_Obj_t *pFanout, Abc_Obj_t *pFanin) {
  Abc_Obj_t *pObj;
  int i;
  Abc_ObjForEachFanin(pFanout, pObj, i) if (pObj == pFanin) return i;
  return -1;
}
