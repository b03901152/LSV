...
system.]
  ...
  Revision    [$Id: abcDfs.c,v 1.00 2005/06/20 00:00:00
alanmi Exp $]
...
void Abc_NtkDfs_rec(Abc_Obj_t *pNode, Vec_Ptr_t *vNodes) {
  ...
  // skip the CI
  if (Abc_ObjIsCi(pNode) || (Abc_NtkIsStrash(pNode->pNtk) &&
                             Abc_AigNodeIsConst(pNode)))
    return;
  assert(Abc_ObjIsNode(pNode) || Abc_ObjIsBox(pNode));
  // visit the transitive fanin of the node
  Abc_ObjForEachFanin(pNode, pFanin, i) {
    //        pFanin = Abc_ObjFanin( pNode,
    //        Abc_ObjFaninNum(pNode)-1-i );
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the DFS ordered array of logic
nodes.]

  Description [Collects only the internal nodes, leaving out
CIs and CO.
  However it marks with the current TravId both CIs and
COs.]
...
Vec_Ptr_t *Abc_NtkDfs(Abc_Ntk_t *pNtk, int fCollectAll) {
  ...
  if (pNtk->nBarBufs2 > 0) {
    Abc_NtkForEachBarBuf(pNtk, pObj, i) {
      ...
      Abc_NtkDfs_rec(Abc_ObjFanin0Ntk(Abc_ObjFanin0(pObj)),
                     vNodes);
  ...
  if (fCollectAll) {
    Abc_NtkForEachNode(
        pNtk, pObj, i) if (!Abc_NodeIsTravIdCurrent(pObj))
...
Vec_Ptr_t *Abc_NtkDfsNodes(Abc_Ntk_t *pNtk,
  ...
  for (i = 0; i < nNodes; i++) {
    if (Abc_NtkIsStrash(pNtk) &&
        Abc_AigNodeIsConst(ppNodes[i]))
      continue;
    if (Abc_ObjIsCo(ppNodes[i])) {
      Abc_NodeSetTravIdCurrent(ppNodes[i]);
      Abc_NtkDfs_rec(
          Abc_ObjFanin0Ntk(Abc_ObjFanin0(ppNodes[i])),
          vNodes);
    } else if (Abc_ObjIsNode(ppNodes[i]) ||
               Abc_ObjIsCi(ppNodes[i]))
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the reverse DFS ordered array of
logic nodes.]

  Description [Collects only the internal nodes, leaving out
CIs/COs.
  However it marks both CIs and COs with the current
TravId.]
...
Vec_Ptr_t *Abc_NtkDfsReverse(Abc_Ntk_t *pNtk) {
  ...
  if (!Abc_NtkIsStrash(pNtk)) {
    Abc_NtkForEachNode(pNtk, pObj,
                       i) if (Abc_NodeIsConst(pObj))
        Vec_PtrPush(vNodes, pObj);
  }
  return vNodes;
}
...
void Abc_NtkDfsReverseNodes_rec(Abc_Obj_t *pNode,
  ...
  pNode->pCopy =
      (Abc_Obj_t *)Vec_PtrEntry(vNodes, pNode->Level);
...
/**Function*************************************************************
  ...
  Description [Collects the levelized array of internal
nodes, leaving out CIs/COs.
  However it marks both CIs and COs with the current
TravId.]
...
nodes, leaving out CIs/COs.
  ...
  Collects only the nodes whose support does not exceed the
set of given CI nodes.]
...
***********************************************************************/
Vec_Ptr_t *Abc_NtkDfsReverseNodesContained(
    Abc_Ntk_t *pNtk, Abc_Obj_t **ppNodes, int nNodes) {
  Vec_Ptr_t *vNodes;
  ...
  for (i = 0; i < nNodes; i++) {
    ...
    assert(pObj->Level == 0);
    pObj->pCopy =
        (Abc_Obj_t *)Vec_PtrEntry(vNodes, pObj->Level);
    Vec_PtrWriteEntry(vNodes, pObj->Level, pObj);
  }
  // iterate through the levels
  for (i = 0; i <= nLevels; i++) {
    // iterate through the nodes on each level
    for (pObj = (Abc_Obj_t *)Vec_PtrEntry(vNodes, i); pObj;
         pObj = pObj->pCopy) {
      // iterate through the fanouts of each node
      Abc_ObjForEachFanout(pObj, pFanout, k) {
        // skip visited nodes
        if (Abc_NodeIsTravIdCurrent(pFanout)) continue;
        // visit the fanins of this fanout
        Abc_ObjForEachFanin(pFanout, pFanin, m) {
          if (!Abc_NodeIsTravIdCurrent(pFanin)) break;
        }
        if (m < Abc_ObjFaninNum(pFanout)) continue;
        ...
        // add to the array
        pFanout->pCopy = (Abc_Obj_t *)Vec_PtrEntry(
            vNodes, pFanout->Level);
        Vec_PtrWriteEntry(vNodes, pFanout->Level, pFanout);
        // handle the COs
        if (Abc_ObjIsCo(pFanout)) pFanout->Level = 0;
      }
    }
  }
  return vNodes;
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the array of nodes and latches
reachable from POs.]
...
***********************************************************************/
void Abc_NtkDfs_iter(Vec_Ptr_t *vStack, Abc_Obj_t *pRoot,
                     Vec_Ptr_t *vNodes) {
  Abc_Obj_t *pNode, *pFanin;
  int iFanin;
  // if this node is already visited, skip
  if (Abc_NodeIsTravIdCurrent(pRoot)) return;
  // mark the node as visited
  Abc_NodeSetTravIdCurrent(pRoot);
  // skip the CI
  if (Abc_ObjIsCi(pRoot) || (Abc_NtkIsStrash(pRoot->pNtk) &&
                             Abc_AigNodeIsConst(pRoot)))
    return;
  // add the CI
  Vec_PtrClear(vStack);
  ...
  while (Vec_PtrSize(vStack) > 0) {
    ...
    // if this node is already visited, skip
    if (Abc_NodeIsTravIdCurrent(pFanin)) continue;
    // mark the node as visited
    Abc_NodeSetTravIdCurrent(pFanin);
    // skip the CI
    if (Abc_ObjIsCi(pFanin) ||
        (Abc_NtkIsStrash(pFanin->pNtk) &&
         Abc_AigNodeIsConst(pFanin)))
      continue;
...
nodes.]
  ...
  Description [Collects only the internal nodes, leaving CIs
and CO.
  However it marks with the current TravId both CIs and
COs.]
...
Vec_Ptr_t *Abc_NtkDfsIter(Abc_Ntk_t *pNtk,
  ...
  Abc_NtkForEachCo(pNtk, pObj, i) {
    Abc_NodeSetTravIdCurrent(pObj);
    Abc_NtkDfs_iter(vStack,
                    Abc_ObjFanin0Ntk(Abc_ObjFanin0(pObj)),
                    vNodes);
  }
  // collect dangling nodes if asked to
  if (fCollectAll) {
...
Vec_Ptr_t *Abc_NtkDfsIterNodes(Abc_Ntk_t *pNtk,
  ...
  vStack = Vec_PtrAlloc(1000);
  Vec_PtrForEachEntry(
      Abc_Obj_t *, vRoots, pObj,
      i) if (!Abc_NodeIsTravIdCurrent(Abc_ObjRegular(pObj)))
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the DFS ordered array of all
objects.]

  Description [This procedure collects everything from POs
to PIs.]
...
Vec_Ptr_t *Abc_NtkDfsHie(Abc_Ntk_t *pNtk, int fCollectAll) {
  ...
  if (fCollectAll) {
    Abc_NtkForEachObj(pNtk, pObj,
                      i) if (!Abc_NodeIsTravIdCurrent(pObj))
        Abc_NtkDfs_rec(pObj, vNodes);
  }
  return vNodes;
}
...
int Abc_NtkIsDfsOrdered(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachNode(pNtk, pNode, i) {
    ...
    Abc_ObjForEachFanin(
        pNode, pFanin,
        k) if (!Abc_NodeIsTravIdCurrent(pFanin)) return 0;
    // check the choices of the node
    if (Abc_NtkIsStrash(pNtk) && Abc_AigNodeIsChoice(pNode))
      for (pFanin = (Abc_Obj_t *)pNode->pData; pFanin;
           pFanin = (Abc_Obj_t *)pFanin->pData)
...
void Abc_NtkDfsNets_rec(Abc_Obj_t *pNet, Vec_Ptr_t *vNets) {
  ...
  if (Abc_NodeIsTravIdCurrent(pNet)) return;
...
void Abc_NtkDfsWithBoxes_rec(Abc_Obj_t *pNode,
  ...
  int i;
  assert(!Abc_ObjIsNet(pNode));
  if (Abc_ObjIsBo(pNode)) pNode = Abc_ObjFanin0(pNode);
  if (Abc_ObjIsPi(pNode)) return;
  assert(Abc_ObjIsNode(pNode) || Abc_ObjIsBox(pNode));
  if (Abc_NodeIsTravIdCurrent(pNode)) return;
  Abc_NodeSetTravIdCurrent(pNode);
  Abc_ObjForEachFanin(pNode, pFanin, i) {
    if (Abc_ObjIsBox(pNode)) pFanin = Abc_ObjFanin0(pFanin);
    assert(Abc_ObjIsNet(pFanin));
    Abc_NtkDfsWithBoxes_rec(Abc_ObjFanin0Ntk(pFanin),
                            vNodes);
  }
  Vec_PtrPush(vNodes, pNode);
}
Vec_Ptr_t *Abc_NtkDfsWithBoxes(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachPo(pNtk, pObj, i) {
    ...
    Abc_NtkDfsWithBoxes_rec(
        Abc_ObjFanin0Ntk(Abc_ObjFanin0(pObj)), vNodes);
  }
  return vNodes;
}
...
void Abc_NtkNodeSupport_rec(Abc_Obj_t *pNode,
  ...
  // visit the transitive fanin of the node
  Abc_ObjForEachFanin(pNode, pFanin, i)
      Abc_NtkNodeSupport_rec(Abc_ObjFanin0Ntk(pFanin),
                             vNodes);
}

/**Function*************************************************************

  Synopsis    [Returns the set of CI nodes in the support of
the given nodes.]
...
Vec_Ptr_t *Abc_NtkSupport(Abc_Ntk_t *pNtk) {
  ...
  // add unused CIs
  Abc_NtkForEachCi(pNtk, pNode,
                   i) if (!Abc_NodeIsTravIdCurrent(pNode))
      Vec_PtrPush(vNodes, pNode);
...
Vec_Ptr_t *Abc_NtkNodeSupport(Abc_Ntk_t *pNtk,
  ...
  for (i = 0; i < nNodes; i++)
    if (Abc_ObjIsCo(ppNodes[i]))
      Abc_NtkNodeSupport_rec(Abc_ObjFanin0(ppNodes[i]),
                             vNodes);
    else
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the set of CI node IDs in the support
of the given node.]
...
void Abc_NtkNodeSupportInt_rec(Abc_Obj_t *pNode,
  ...
  // visit the transitive fanin of the node
  Abc_ObjForEachFanin(pNode, pFanin, i)
      Abc_NtkNodeSupportInt_rec(Abc_ObjFanin0Ntk(pFanin),
                                vNodes);
}
Vec_Int_t *Abc_NtkNodeSupportInt(Abc_Ntk_t *pNtk, int iCo) {
  ...
  Abc_Obj_t *pObj;
  int i;
  if (iCo < 0 || iCo >= Abc_NtkCoNum(pNtk)) return NULL;
  // save node indices in the CI nodes
  Abc_NtkForEachCi(pNtk, pObj, i) pObj->iTemp = i;
  // collect the indexes of CI nodes in the TFI of the CO
  // node
  Abc_NtkIncrementTravId(pNtk);
...
int Abc_NtkFunctionalIsoGia_rec(Gia_Man_t *pNew,
  ...
  if (Abc_NodeIsTravIdCurrent(pNode) ||
      Abc_ObjFaninNum(pNode) == 0 || Abc_ObjIsCi(pNode))
    return pNode->iTemp;
  assert(Abc_ObjIsNode(pNode));
  Abc_NodeSetTravIdCurrent(pNode);
  iLit0 = Abc_NtkFunctionalIsoGia_rec(pNew,
                                      Abc_ObjFanin0(pNode));
  iLit1 = Abc_NtkFunctionalIsoGia_rec(pNew,
                                      Abc_ObjFanin1(pNode));
...
Gia_Man_t *Abc_NtkFunctionalIsoGia(Abc_Ntk_t *pNtk,
  ...
  if (Vec_IntSize(vSupp1) == Vec_IntSize(vSupp2)) {
    ...
    if (fCommon) {
      Vec_Int_t *vCommon =
          Vec_IntAlloc(Vec_IntSize(vSupp1));
    ...
    Vec_IntForEachEntry(vSupp1, iCi, i) Abc_NtkCi(pNtk, iCi)
        ->iTemp = Gia_ManAppendCi(pNew);
    // create the first cone
    Abc_NtkIncrementTravId(pNtk);
    pObj = Abc_NtkCo(pNtk, iCo1);
    iLit1 = Abc_NtkFunctionalIsoGia_rec(
        pNew, Abc_ObjFanin0(pObj));
    iLit1 = Abc_LitNotCond(iLit1, Abc_ObjFaninC0(pObj));
    // primary inputs
    Vec_IntForEachEntry(vSupp2, iCi, i) Abc_NtkCi(pNtk, iCi)
        ->iTemp = Gia_ManCiLit(pNew, i);
    // create the second cone
    Abc_NtkIncrementTravId(pNtk);
    pObj = Abc_NtkCo(pNtk, iCo2);
    iLit2 = Abc_NtkFunctionalIsoGia_rec(
        pNew, Abc_ObjFanin0(pObj));
...
int Abc_NtkFunctionalIsoInt(Abc_Ntk_t *pNtk, int iCo1,
                            int iCo2, int fCommon) {
  Gia_Man_t *pGia;
  int Value;
  assert(Abc_NtkIsStrash(pNtk));
  if (iCo1 < 0 || iCo1 >= Abc_NtkCoNum(pNtk)) return 0;
  if (iCo2 < 0 || iCo2 >= Abc_NtkCoNum(pNtk)) return 0;
  pGia = Abc_NtkFunctionalIsoGia(pNtk, iCo1, iCo2, fCommon);
  if (pGia == NULL) return 0;
  Value = Cec_ManVerifySimple(pGia);
  Gia_ManStop(pGia);
  return (int)(Value == 1);
}
int Abc_NtkFunctionalIso(Abc_Ntk_t *pNtk, int iCo1,
                         int iCo2, int fCommon) {
  Abc_Ntk_t *pNtkNew;
  int Result;
  if (Abc_NtkIsStrash(pNtk))
  ...
  Result =
      Abc_NtkFunctionalIsoInt(pNtkNew, iCo1, iCo2, fCommon);
...
int Abc_ObjSuppSize_rec(Abc_Obj_t *pObj) {
  Abc_Obj_t *pFanin;
  int i, Counter = 0;
  if (Abc_NodeIsTravIdCurrent(pObj)) return 0;
  Abc_NodeSetTravIdCurrent(pObj);
  if (Abc_ObjIsPi(pObj)) return 1;
  assert(Abc_ObjIsNode(pObj) || Abc_ObjIsBox(pObj));
  Abc_ObjForEachFanin(pObj, pFanin, i) Counter +=
      Abc_ObjSuppSize_rec(pFanin);
  return Counter;
}
/**Function*************************************************************
...
int Abc_NtkSuppSizeTest(Abc_Ntk_t *p) {
  ...
  Abc_NtkForEachObj(p, pObj, i) if (Abc_ObjIsNode(pObj))
...
/**Function*************************************************************
  ...
  Synopsis    [Computes the sum total of supports of all
outputs.]
...
Vec_Ptr_t *Abc_AigDfs(Abc_Ntk_t *pNtk, int fCollectAll,
  ...
  Abc_NtkForEachCo(pNtk, pNode, i) {
    ...
    Abc_NodeSetTravIdCurrent(pNode);
    if (fCollectCos) Vec_PtrPush(vNodes, pNode);
  }
  // collect dangling nodes if asked to
  if (fCollectAll) {
    Abc_NtkForEachNode(
        pNtk, pNode, i) if (!Abc_NodeIsTravIdCurrent(pNode))
...
Vec_Ptr_t *Abc_AigDfsMap(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachCo(pNtk, pNode, i) {
    if (i < Abc_NtkCoNum(pNtk) - pNtk->nBarBufs) continue;
    Abc_AigDfs_rec(Abc_ObjFanin0(pNode), vNodes);
    Abc_NodeSetTravIdCurrent(pNode);
  ...
  Abc_NtkForEachCo(pNtk, pNode, i) {
    if (i >= Abc_NtkCoNum(pNtk) - pNtk->nBarBufs) break;
    Abc_AigDfs_rec(Abc_ObjFanin0(pNode), vNodes);
...
/**Function*************************************************************
  ...
  Synopsis    [Recursively counts the number of logic levels
of one node.]
...
int Abc_NtkLevel_rec(Abc_Obj_t *pNode) {
  ...
  assert(!Abc_ObjIsNet(pNode));
  // skip the PI
  if (Abc_ObjIsCi(pNode)) return pNode->Level;
  assert(Abc_ObjIsNode(pNode) ||
         pNode->Type == ABC_OBJ_CONST1);
  // if this node is already visited, return
  if (Abc_NodeIsTravIdCurrent(pNode)) return pNode->Level;
  // mark the node as visited
  Abc_NodeSetTravIdCurrent(pNode);
...
int Abc_NtkLevelReverse_rec(Abc_Obj_t *pNode) {
  ...
  assert(!Abc_ObjIsNet(pNode));
  // skip the PI
  if (Abc_ObjIsCo(pNode)) return pNode->Level;
  assert(Abc_ObjIsNode(pNode) ||
         pNode->Type == ABC_OBJ_CONST1);
  // if this node is already visited, return
  if (Abc_NodeIsTravIdCurrent(pNode)) return pNode->Level;
  ...
  Abc_ObjForEachFanout(pNode, pNext, i) {
    Level =
        Abc_NtkLevelReverse_rec(Abc_ObjFanout0Ntk(pNext));
    if (pNode->Level < (unsigned)Level)
      pNode->Level = Level;
  }
  if (Abc_ObjFaninNum(pNode) > 0 && !Abc_ObjIsBarBuf(pNode))
...
/**Function*************************************************************
  ...
  Synopsis    [Computes the number of logic levels not
counting PIs/POs.]
...
int Abc_NtkLevel(Abc_Ntk_t *pNtk) {
  ...
  if (pNtk->pManTime == NULL || pNtk->AndGateDelay <= 0)
    Abc_NtkForEachCi(pNtk, pNode, i) pNode->Level = 0;
  else
    Abc_NtkForEachCi(pNtk, pNode, i) pNode->Level =
        (int)(Abc_MaxFloat(
                  0, Abc_NodeReadArrivalWorst(pNode)) /
              pNtk->AndGateDelay);
  ...
  if (pNtk->nBarBufs == 0) {
    ...
    Abc_NtkForEachLiPo(pNtk, pNode, i) {
      ...
      // transfer the delay
      if (i < pNtk->nBarBufs)
        Abc_ObjFanout0(Abc_ObjFanout0(pNode))->Level =
            pDriver->Level;
    }
  }
  return LevelsMax;
...
int Abc_NtkLevelReverse(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachCo(pNtk, pNode, i) pNode->Level = 0;
  // perform the traversal
  LevelsMax = 0;
  Abc_NtkIncrementTravId(pNtk);
  Abc_NtkForEachNode(pNtk, pNode, i) {
...
int Abc_NtkIsAcyclic_rec(Abc_Obj_t *pNode) {
  ...
  if (Abc_NodeIsTravIdCurrent(pNode)) {
    fprintf(stdout,
            ...
            Abc_NtkName(pNtk));
    fprintf(stdout,
            "Node \"%s\" is encountered twice on the "
            "following path to the COs:\n",
            Abc_ObjName(pNode));
    return 0;
  }
  ...
  Abc_ObjForEachFanin(pNode, pFanin, i) {
    ...
    if (Abc_NodeIsTravIdPrevious(pFanin)) continue;
    // traverse the fanin's cone searching for the loop
    if ((fAcyclic = Abc_NtkIsAcyclic_rec(pFanin))) continue;
...
/**Function*************************************************************
  ...
  Description [This procedure is based on the idea suggested
by Donald Chai.
  As we traverse the network and visit the nodes, we need to
distinquish
  three types of nodes: (1) those that are visited for the
first time,
  (2) those that have been visited in this traversal but are
currently not
  on the traversal path, (3) those that have been visited
and are currently
  on the travesal path. When the node of type (3) is
encountered, it means
  that there is a combinational loop. To mark the three
types of nodes,
...
int Abc_NtkIsAcyclic(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkIncrementTravId(pNtk);
  Abc_NtkIncrementTravId(pNtk);
  // pNode->TravId == pNet->nTravIds      means "pNode is on
  // the path"
  // pNode->TravId == pNet->nTravIds - 1  means "pNode is
  // visited but is not on the path"
  // pNode->TravId <  pNet->nTravIds - 1  means "pNode is
  // not visited"
  ...
  Abc_NtkForEachCo(pNtk, pNode, i) {
    ...
    if (Abc_NodeIsTravIdPrevious(pNode)) continue;
    // traverse the output logic cone
    if ((fAcyclic = Abc_NtkIsAcyclic_rec(pNode))) continue;
...
int Abc_NtkIsAcyclicWithBoxes_rec(Abc_Obj_t *pNode) {
  ...
  if (Abc_NodeIsTravIdCurrent(pNode)) {
    ...
    if (Abc_ObjIsBox(pNode))
      fprintf(stdout,
              "Box \"%s\" is encountered twice on the "
              "following path to the COs:\n",
              Abc_ObjName(pNode));
    else
      fprintf(stdout,
  ...
  Abc_ObjForEachFanin(pNode, pFanin, i) {
    if (Abc_ObjIsBox(pNode)) pFanin = Abc_ObjFanin0(pFanin);
    pFanin = Abc_ObjFanin0Ntk(pFanin);
    if (Abc_ObjIsBo(pFanin)) pFanin = Abc_ObjFanin0(pFanin);
    // check if the fanin is visited
    if (Abc_ObjIsPi(pFanin) || Abc_ObjIsLatch(pFanin) ||
        Abc_ObjIsBlackbox(pFanin))
      continue;
    ...
    // return as soon as the loop is detected
    fprintf(stdout, " %s ->",
            Abc_ObjName(Abc_ObjIsBox(pFanin)
                            ? pFanin
                            : Abc_ObjFanout0(pFanin)));
    return 0;
  }
  // mark this node as a visited node
  assert(Abc_ObjIsNode(pNode) || Abc_ObjIsBox(pNode));
...
int Abc_NodeSetChoiceLevel_rec(Abc_Obj_t *pNode,
  ...
  Level1 = Abc_NodeSetChoiceLevel_rec(Abc_ObjFanin0(pNode),
                                      fMaximum);
  Level2 = Abc_NodeSetChoiceLevel_rec(Abc_ObjFanin1(pNode),
                                      fMaximum);
  Level = 1 + Abc_MaxInt(Level1, Level2);
  if (pNode->pData) {
    LevelE = Abc_NodeSetChoiceLevel_rec(
        (Abc_Obj_t *)pNode->pData, fMaximum);
    if (fMaximum)
      Level = Abc_MaxInt(Level, LevelE);
    else
      Level = Abc_MinInt(Level, LevelE);
    // set the level of all equivalent nodes to be the same
    // minimum
    for (pTemp = (Abc_Obj_t *)pNode->pData; pTemp;
         pTemp = (Abc_Obj_t *)pTemp->pData)
...
/**Function*************************************************************
  ...
  Synopsis    [Resets the levels of the nodes in the choice
graph.]

  Description [Makes the level of the choice nodes to be
equal to the
  maximum of the level of the nodes in the equivalence
class. This way
  sorting by level leads to the reverse topological order,
which is
...
int Abc_AigSetChoiceLevels(Abc_Ntk_t *pNtk) {
  ...
  Abc_NtkForEachCo(pNtk, pObj, i) {
    LevelCur =
        Abc_NodeSetChoiceLevel_rec(Abc_ObjFanin0(pObj), 1);
...
/**Function*************************************************************
  ...
  Synopsis    [Returns nodes by level from the smallest to
the largest.]

  Description [Correctly handles the case of choice nodes,
by first
  spreading them out across several levels and then
collecting.]
...
Vec_Ptr_t *Abc_AigGetLevelizedOrder(Abc_Ntk_t *pNtk,
  ...
  Abc_NtkForEachNode(pNtk, pNode, i) {
    ppHead = ((Abc_Obj_t **)vLevels->pArray) +
  ...
  Vec_PtrForEachEntryStart(
      Abc_Obj_t *, vLevels, pNode, i,
      !fCollectCis) for (; pNode; pNode = pNode->pCopy)
      Vec_PtrPush(vNodes, pNode);
  Vec_PtrFree(vLevels);
  return vNodes;
}
...
int Abc_ObjSugraphSize(Abc_Obj_t *pObj) {
  if (Abc_ObjIsCi(pObj)) return 0;
  if (Abc_ObjFanoutNum(pObj) > 1) return 0;
...
int Abc_NtkPrintSubraphSizes(Abc_Ntk_t *pNtk) {
  ...
  assert(Abc_NtkIsStrash(pNtk));
  Abc_NtkForEachNode(pNtk, pObj,
                     i) if (Abc_ObjFanoutNum(pObj) > 1 &&
                            !Abc_NodeIsExorType(pObj))
      printf("%d(%d) ",
             1 + Abc_ObjSugraphSize(Abc_ObjFanin0(pObj)) +
                 Abc_ObjSugraphSize(Abc_ObjFanin1(pObj)),
